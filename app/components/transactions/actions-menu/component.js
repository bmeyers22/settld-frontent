...
export default Ember.Component.extend({
  ...
  unpaidInvoice: Ember.computed('model.invoices.@each.paid', 'model.invoices.@each.paymentPending', function () {
    if (this.get('model.user') === this.get('currentSession.authUser')) {
      return false;
    }
    let invoice = this.get('transactionsService').filterInvoicesByStatus(this.get('model'), 'paid', false, this.get('currentSession.authUser.id'));
  ...
  pendingInvoice: Ember.computed('model.invoices.@each.paymentPending', function () {
    ...
    return this.get('transactionsService').filterInvoicesByStatus(this.get('model'), 'paymentPending', true, this.get('currentSession.authUser.id'));
  }),
  owedInvoices: Ember.computed('model', function () {
    return this.get('transactionsService').getOwedInvoicesByUser(this.get('model'), this.get('currentSession.authUser.id'));
  }),
  ...
  canDelete: Ember.computed(function () {
    return this.get('model.user') === this.get('currentSession.authUser')
